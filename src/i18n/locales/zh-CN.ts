...
export const zhCN: TranslationCatalog = {
  ...
  timeline: {
    ...
    segmentDescription:
      "导出顺序：第 {{order}} 个，共 {{total}} 个。出点是片段后的第一帧。",
    segmentTooltip: {
  ...
  mediaError: {
    ...
    metadataFailed: "无法读取文件元数据。",
    unsafeMetadata: "文件元数据超出安全限制。",
    appDataUnavailable: "应用程序数据目录不可用。",
    ffmpegPairMissing: "无法找到所需的 FFmpeg 或 ffprobe 程序。",
    ffprobeSpawnFailed: "无法启动 ffprobe 进程。",
    ffprobeProcessFailed: "ffprobe 无法检查该媒体文件。",
    ffprobeParseFailed: "无法解析 ffprobe 输出。",
    ffprobeTimedOut:
      "ffprobe 未在限定时间内响应。该文件可能位于已停止响应的驱动器或共享位置上。",
    assetScopeDenied: "资源协议拒绝访问该媒体文件。",
    commandExecutionFailed: "无法运行媒体导入命令。",
    dialogFailed: "无法打开文件选择对话框。",
    unknown: "导入媒体时发生未知错误。",
  },
  playbackError: {
    playbackFailed: "无法开始播放。",
    seekFailed: "无法跳转到指定位置。",
  },
  statusBar: {
    source: {
    ...
    approximatePosition: "近似位置 · 无法标记",
    approximatePositionDetail: "播放头跟随系统播放器的时钟，而不是精确的帧时间戳。",
    approximatePositionMarks:
      "此视频无法使用标记入点、标记出点和分割。如果将视频转换为 MP4 并打开转换后的文件，这些操作可能会变为可用。",
  ...
  settings: {
    ...
    ffmpeg: {
      ...
      hint: "可以选择文件夹，也可以选择单个文件。如有可能，请选择同时包含 FFmpeg 和 ffprobe 的文件夹。",
      source: {
        user: "由您选择：",
        automatic: "自动检测到：",
  ...
  ffmpegError: {
    ...
    versionParseFailed: "无法解析 FFmpeg 版本输出。",
    encoderListParseFailed: "无法解析 FFmpeg 编码器列表。",
    cacheUnavailable: "FFmpeg 能力缓存不可用。",
    commandExecutionFailed: "无法运行 FFmpeg 能力探测命令。",
  ...
  settingsError: {
    ...
    settingsConflict:
      "在此副本加载设置之后，另一个窗口或另一个 QuipClip 副本保存了设置。QuipClip 未保存您的更改，以免它覆盖另一处更改。当前显示的设置已是磁盘上的设置。请重新应用您的更改。",
    backupFailed: "无法备份现有设置文件。",
    invalidPath: "配置的 FFmpeg 路径无效。",
    commandExecutionFailed: "无法运行设置命令。",
    dialogFailed: "无法打开文件选择对话框。",
    unknown: "处理设置时发生未知错误。",
  },
  export: {
  ...
  exportError: {
    ...
    invalidSegment: "一个或多个导出片段的入点或出点无效。",
    ...
    outputRenameFailed: "无法将完成的视频保存到目标路径。",
    canceled: "导出已停止。",
    commandExecutionFailed: "无法运行导出命令。",
    exportAlreadyRunning: "已有导出任务正在运行。",
    dialogFailed: "无法打开导出保存对话框。",
